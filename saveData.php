<?php
## Do this at top, so that if errors occur during saving, rest of settings won't be updated
...
$currentFileKey = max(array_keys($import_list));
$import_list[$currentFileKey] = $start_import;
...
$import_checked[$currentFileKey] = filter_var($_REQUEST['checkExisting'], FILTER_VALIDATE_BOOLEAN);
...
$import_chkerrors[$currentFileKey] = filter_var($_REQUEST['checkErrors'], FILTER_VALIDATE_BOOLEAN);
...
$import_check_started[$currentFileKey] = $start_import;
...
$import_continue[$currentFileKey] = $start_import;
...
$import_cancel[$currentFileKey] = false;
...
$import_cancel_check[$currentFileKey] = false;
...
$import_number[$currentFileKey] = $total_import;
...
$import_delimiter[$currentFileKey] = $_REQUEST['csvDelimiter'];
...
$import_datetime[$currentFileKey] = $_REQUEST['dateFormat'];
...
$import_overwrite[$currentFileKey] = filter_var($_REQUEST['checkOverwrite'], FILTER_VALIDATE_BOOLEAN);
$module->setProjectSetting('import-overwrite', $import_overwrite);

$import_rows = empty($module->getProjectSetting('import-current-row'))?array():$module->getProjectSetting('import-current-row');
$import_rows[$currentFileKey] = 0;
$module->setProjectSetting('import-current-row', $import_rows);

$import_rows = empty($module->getProjectSetting('import-check-row'))?array():$module->getProjectSetting('import-check-row');
$import_rows[$currentFileKey] = 1;
$module->setProjectSetting('import-check-row', $import_rows);
